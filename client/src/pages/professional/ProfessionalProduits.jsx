...
const ProfessionalProduits = () => {
  const user = useSelector((state) => state.auth.user);
  const professionalId = user?.id; // For professionals, their account ID is their identifier

  const { data: products = [], isLoading, isError, refetch } = useGetCompanyProductsQuery(professionalId, { 
    skip: !professionalId, 
  ...
  const handleRemoveImage = (index) => {
    ...
    // In a real app, you'd also remove from formData.imagesProduct if it's a new file
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Erreur lors de l'enregistrement du produit : " + (err.data?.message || err.message));
    }
  };
    ...
    <div style={{ padding: "40px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
        <div>
          ...
          <p style={{ color: "#64748b", marginTop: "5px" }}>Gérez votre catalogue de produits personnels.</p>
        </div>
        <button 
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: '100px', backgroundColor: 'white', borderRadius: '24px', border: '2px dashed #e2e8f0' }}>
          ...
          <p style={{ color: '#94a3b8' }}>{searchQuery ? "Aucun résultat." : "Commencez par ajouter votre premier produit."}</p>
        </div>
      ) : (
            ...
            <div key={product._id} style={{ backgroundColor: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '15px' }}>
                  ...
                  <span style={{ fontSize: '13px', color: product.stock > 0 ? '#10b981' : '#ef4444', fontWeight: '600' }}>{product.stock > 0 ? `Stock: ${product.stock}` : "Rupture"}</span>
                </div>
              </div>
            </div>
          ))}
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '14px', fontWeight: '600', color: '#475569' }}>Nom *</label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input required style={{ padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input type="number" style={{ padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} value={formData.stock} onChange={(e) => setFormData({...formData, stock: e.target.value})} />
                </div>
              </div>
...
export default ProfessionalProduits;
