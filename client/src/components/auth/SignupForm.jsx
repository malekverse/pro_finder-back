  ...
  <div className={styles.stepContent}>
    ...
    <div className={styles.locationGridStyled}>
      <Autocomplete
        ...
        options={regions}
        getOptionLabel={(option) => option.name} // affiche le nom de la région
        value={
          companyInputs.region_id
            ? regions.find(r => r._id === companyInputs.region_id)
            : null
        }
        onSelect={(region) => {
     ...
     <Autocomplete
      ...
      options={cities}
      getOptionLabel={(option) => option.name} // pour afficher le nom de la ville
      value={
        companyInputs.city_id
          ? cities.find(c => c._id === companyInputs.city_id)
          : null
      }
      onSelect={(city) => {
        console.log("belhassen "+cities);
        setCompanyInputs(prev => ({
          ...prev,
          city_id: city ? city._id : ""
        }));
      }}
    />
    </div>
